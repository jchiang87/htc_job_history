...
def plot_time_history(df0, label=None, linestyle=None, color=None, alpha=0.5,
    ...
    if make_plot:
        plt.plot(df['datetime'], df['num_jobs'], label=label,
                 linestyle=linestyle, color=color, alpha=alpha)
    return np.trapezoid(df["num_jobs"], df["times"])/3600.  # convert to hours
